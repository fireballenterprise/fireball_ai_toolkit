...
Verbs (apt-style):
  ...
  apply       clobber .fireball_ai_toolkit/ + modules/fireball_ai_toolkit/ + … from the installed package, then render
  upgrade     update, then apply — the whole "take the new toolkit" in one
  sync        apply, but stop first if .fireball_ai_toolkit/ has local hand-edits
  contribute  open a PR to the toolkit with this repo's .fireball_ai_toolkit/ edits
...
def apply(context):  # noqa: ARG001
    """Clobber .fireball_ai_toolkit/ + modules/fireball_ai_toolkit/ + … from the installed package, then render."""
...
def upgrade(context, force=False):
    ...
    Stops if .fireball_ai_toolkit/ has local hand-edits (pass --force to clobber them).
    """
    update(context)
    ...
    # script picks up the just-installed version. Same `sync` semantics: stop on local .fireball_ai_toolkit/
    # edits unless --force.
    result = context.run(
    ...
    if result.exited == 2:
        raise SystemExit(2)  # .fireball_ai_toolkit/ has local edits — re-run with --force to clobber them
    if result.exited != 0:
...
def sync(context, force=False):  # noqa: ARG001
    """Inspect .fireball_ai_toolkit/ for local edits; if clean (or --force), apply (clobber + render)."""
...
def contribute(context, branch=None, toolkit_repo=None):  # noqa: ARG001
    """Open a PR against fireball_ai_toolkit with this repo's .fireball_ai_toolkit/ edits.
...
def check(context):  # noqa: ARG001
    """Read-only drift gate — fail if any generated provider file (or .fireball_ai_toolkit/) is stale."""
    _check(Path.cwd())
    print("No drift.")
...
def download(context):
    ...
    print("note: `ai_toolkit.download` is now `ai_toolkit.apply`")
    apply(context)
...
def upload(context, branch=None, toolkit_repo=None):
    ...
    print("note: `ai_toolkit.upload` is now `ai_toolkit.contribute`")
