"""modules.fireball_ai_toolkit.backlog.route — the thin `/backlog <subcommand>` dispatcher."""

import pytest
from modules.fireball_ai_toolkit.backlog import route as backlog_route
...
def _tail(command):
    """The `-m modules.fireball_ai_toolkit.backlog.<sub> ...` portion of the spawned command."""
    return command[command.index("-m") + 1 :]


def test_bare_invocation_defaults_to_list_all(monkeypatch, dispatched):
    assert _run(monkeypatch, "") == 0
    assert _tail(dispatched[0]) == ["modules.fireball_ai_toolkit.backlog.list", "--all"]
...
def test_known_subcommand_passes_through(monkeypatch, dispatched):
    ...
    assert _tail(dispatched[0]) == ["modules.fireball_ai_toolkit.backlog.list", "--repo", "toolkit"]
...
def test_add_bug_shorthand_expands_to_type_flag(monkeypatch, dispatched):
    ...
    assert _tail(dispatched[0]) == [
        "modules.fireball_ai_toolkit.backlog.add",
        "--type",
        "bug",
        "--repo",
        "toolkit",
