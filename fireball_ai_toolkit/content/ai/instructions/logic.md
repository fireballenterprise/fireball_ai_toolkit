...
modules/*/            All business logic. Reusable, testable. The only place anything
        ...
        └── tasks/*.py (invoke)  Thin CLI wrapper too, reached via `invoke <task>`. Used for
                                  ...
                                  — see `.fireball_ai_toolkit/toolkit/instructions/tasks.md`.
        ↑
...
- A **`/slash-command`** (`/repo`, `/push`, `/backlog`, …) is not an executable — never type one
  into a terminal. Its behaviour is the `.fireball_ai_toolkit/toolkit/commands/<name>.md` (or `.fireball_ai_toolkit/<repo>/commands/`)
...
`content/commands/`, mirrored into each consuming repo as `.fireball_ai_toolkit/toolkit/commands/`). Those bodies are
the single source of truth every AI tool reads for command *behavior* (see
`.fireball_ai_toolkit/toolkit/instructions/ai_commands.md` for authoring them). If a decision isn't written down in a
command body or a standing rule under `.fireball_ai_toolkit/toolkit/instructions/` (or `.fireball_ai_toolkit/<repo>/instructions/`), an
...
`.fireball_ai_toolkit/toolkit/commands/<slug>.md` file and running the same
...
rules live under `.fireball_ai_toolkit/toolkit/instructions/`, no business logic and no decision logic is tied to any
...
and mirrored into each consuming repo as **`.fireball_ai_toolkit/toolkit/`**; repo-specific ones live in
**`.fireball_ai_toolkit/<repo>/`**. A generator renders both layers into every provider's native files.

**Every generated provider file is a pointer stub** — provider-specific frontmatter (`applyTo`,
`allowed-tools`, `name`, …) plus a single line back to `.fireball_ai_toolkit/toolkit/<kind>/<slug>.md` (or
`.fireball_ai_toolkit/<repo>/…`). No canonical body text is inlined anywhere. This includes Copilot's
...
`AGENTS.md`, `CLAUDE.md`). Edit `.fireball_ai_toolkit/toolkit/` (via the toolkit) or `.fireball_ai_toolkit/<repo>/` and run
...
|----------|-------|-------|
| **GitHub Copilot** | `.github/copilot-instructions.md` + `.github/instructions/*.instructions.md` (pointer stubs) | the `applyTo` glob auto-applies the stub; Copilot must follow the pointer to `.fireball_ai_toolkit/toolkit/instructions/<slug>.md` |
| **Claude Code** | `CLAUDE.md` → `AGENTS.md` → `.fireball_ai_toolkit/toolkit/ + .fireball_ai_toolkit/<repo>/instructions/`; `.claude/commands/` + `.claude/skills/` (stubs → `.fireball_ai_toolkit/`) | |
| **Fireball Sidecar** | `AGENTS.md` + `.sidecar/` stubs → `.fireball_ai_toolkit/toolkit/ + .fireball_ai_toolkit/<repo>/` | |
| **Codex / other AGENTS.md tools** | `AGENTS.md` → `.fireball_ai_toolkit/toolkit/ + .fireball_ai_toolkit/<repo>/instructions/` | |
...
## Documentation
- `.fireball_ai_toolkit/toolkit/instructions/logic.md` — this file (stack + provider architecture)
- `.fireball_ai_toolkit/toolkit/instructions/ai_commands.md` — canonical command / instruction authoring
- `.fireball_ai_toolkit/toolkit/instructions/ai_skills.md` — skill file conventions
- `.fireball_ai_toolkit/toolkit/instructions/tasks.md` — invoke task runner (plain CLI automation, no AI)
- `.fireball_ai_toolkit/toolkit/instructions/modules.md` — Python module architecture and layout conventions
- `.fireball_ai_toolkit/toolkit/instructions/tests.md` — testing requirements and workflow

Repo-specific instructions live in the consuming repo's `.fireball_ai_toolkit/<repo>/instructions/` — the generated
