...
`modules/fireball_ai_toolkit/docs/lib/change_logs.py`'s `CHANGELOG_CATEGORIES` tuple is empty by default. Add a
...
<category>:
  <name>:
    version: 1.0.0
    latest_changes:
      author: Ada Lovelace    # git config user.name — modules.fireball_ai_toolkit.setup.properties.get_git_author()
      date: 2026-08-11
...
From `modules/fireball_ai_toolkit/docs/lib/change_logs.py`'s `expected_entry_text()`:
...
## Sync Mechanism
`modules/fireball_ai_toolkit/docs/lib/change_logs.py`:
