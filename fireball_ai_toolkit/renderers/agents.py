...
* ``AGENTS.md`` — the tool-neutral **index**: a short preamble, one bullet per instruction linking
  to its canonical ``.fireball_ai_toolkit/toolkit/`` (or ``.fireball_ai_toolkit/<repo>/``) file, and the source-of-truth note.
...
_PREAMBLE = (
    "This repo's rules for any AI coding tool are authored once under `.fireball_ai_toolkit/toolkit/` (synced from "
    "`fireball_ai_toolkit`) and `.fireball_ai_toolkit/{local}/` (this repo's own). Every `.github/`, `.claude/`, "
    "`.clinerules/`, and `.sidecar/` file is a generated pointer back to them. Read the relevant "
    "canonical file before making changes:"
)

_SOURCE_OF_TRUTH = (
    "**Source of truth: `.fireball_ai_toolkit/toolkit/` and `.fireball_ai_toolkit/{local}/`** — all rules, slash commands, and skills "
...
def _canonical(bundle: ContentBundle, slug: str) -> str:
    ...
    return f".fireball_ai_toolkit/{layer}/instructions/{slug}.md"
